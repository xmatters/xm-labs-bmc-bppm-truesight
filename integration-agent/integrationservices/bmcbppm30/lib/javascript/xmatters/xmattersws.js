...
var xMattersWS = WSUtil.extend({
  ...
  XMATTERS_WSDL_DEFN: "integrationservices/lib/resources/soap/xmatters/v4.1/AlarmPoint-soapui-project.xml",
  XM_SOAP_VERSION: "1_1",
   ...
   */
  sendDelAPXML: function(incidentId, apiaProcessGroup)
  {
    ...
    log.debug("Built DEL apxml: " + apxml);
    ...
    {
      log.error("Caught exception sending del to xMatters for Incident ID: " + incidentId);
      throw e;
    }
    
    log.debug("DEL apxml sent to xMatters");
  },
  ...
  submitAPXML: function(apxml, delExistingEvents)  
  {
    ...
    if (recentlyReceived <= 1) 
    {
      ...
      {
        this.sendDelAPXML(apxml.getValue("incident_id"), apiaProcessGroup);      
      }
