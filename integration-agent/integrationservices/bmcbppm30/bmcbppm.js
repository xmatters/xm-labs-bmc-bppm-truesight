...
function apia_event(form)
{
	...
	form.recipients = [{"targetName":recipients}];
...
function apia_http(httpRequestProperties, httpResponse)
{
    ...
    // 3. Create properties
    event.properties = {};
    IALOG.info("String to be converted to array: " + String(data.*::properties.*::building));
    ...
    if (XMUtil.deduplicate(event.properties)) {
        return;
    }
...
function terminateEventByIncident(incident_id)
{
	...
	try
	{
		...
		var response = XMIO.get(REST_SERVICE_URL + "/events?status=ACTIVE&properties=incident_id%23en%3D" + incident_id);
		IALOG.debug("terminateEventByIncident - XMIO.get response.body: " + response.body);
		...
		if (response.body.substring(0, matchStr.length).equalsIgnoreCase(matchStr)) {
			...
			IALOG.debug("terminateEventByIncident - Event URL: " + REST_SERVICE_URL + eventUrl);
			...
			IALOG.debug("terminateEventByIncident - XMIO.put response.body: " + response.body);
		}
